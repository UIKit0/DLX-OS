...
PCB		*currentPCB;
extern int counter[MEM_MAX_PAGES];
// List of free PCBs.
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
   ...
   // printf("initialize test\n");
    for(j=0;j<MEM_PTSIZE;j++){
        pcbs[i].pagetable[j]= MemoryInitPte(j);
        pcbs[i].npages=0;
    }
...
void ProcessFreeResources (PCB *pcb) {
  ...
  //------------------------------------------------------------
  //printf("free memory test\n");
  //free code and text
  for(i=0;i<MEM_PTSIZE;i++){
   if(pcb->pagetable[i]!=0){
   page=((pcb->pagetable[i])&(0xfffff000))>>(MEM_L1FIELD_FIRST_BITNUM);//get the page number
   counter[page]-=1;
 //  printf("Freeing memory:counter[0x%x]=%d\n",page,counter[page]);
     if(counter[page]==0){
//     printf("freeing PTE 0x%x\n",pcb->pagetable[i]);
     MemoryFreePte (pcb->pagetable[i]);
   }
  }
 }
  //free system stack
//  printf("free system stack ");
  MemoryFreePage (pcb->sysStackArea /(MEM_PAGESIZE));
  counter[(pcb->sysStackArea/(MEM_PAGESIZE))]-=1;
//  printf("counter[0x%x]= %d\n",(pcb->sysStackArea/(MEM_PAGESIZE)),counter[(pcb->sysStackArea/(MEM_PAGESIZE))]);
  //free user stack
/*  i=(MEM_PTSIZE)-1;
  while(pcb->pagetable[i]!=0)
  {
    printf("free user stack 0x%x\n",pcb->pagetable[i]);
    MemoryFreePte (pcb->pagetable[i]);
    i--;
  }*/
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  int newPage;             // memory catch variable
  ...
  //--------------------------------------------------------
   //set up 4 pages for code and data
    for(i=0;i<4;i++){
   ...
   counter[newPage] +=1;
   //printf("allocate a page for code and data, PTE is 0x%x\n",pcb->pagetable[i]);
  // printf("counter[0x%x] is %d\n",newPage,counter[newPage]);
  }
  
   //set up 1 page for user stack
   newPage = MemoryAllocPage ();
   if (newPage == MEM_FAIL) {
      printf("aFATAL:couldn't allocate memory - no free pages!\n");
      ProcessKill();
   ...
   pcb->npages +=1;
   pcb->pagetable[(MEM_PTSIZE)-1]=MemorySetupPte(newPage);
   counter[newPage] +=1;
 //  printf("counter[0x%x]= %d\n",newPage,counter[newPage]);
 //  printf("allocate a page for user statck,page number is 0x%x, PTE is 0x%x\n",(MEM_PTSIZE)-1,pcb->pagetable[(MEM_PTSIZE)-1]);
      
   newPage = MemoryAllocPage ();//set up page for system stack
   if (newPage == MEM_FAIL) {
      printf("aFATAL:couldn't allocate system stack - no free pages!\n");
      ProcessKill();
   }
   counter[newPage] +=1;
   pcb->sysStackArea = newPage * MEM_PAGESIZE;
   stackframe =(uint32*)((pcb->sysStackArea+(MEM_PAGESIZE))-4); //to make it 4-byte alligned
 //  printf("allocate a page for system stack,sysStackArea is 0x%x\n",pcb->sysStackArea);
 //  printf("counter[0x%x]= %d\n",newPage,counter[newPage]);
  ...
  //----------------------------------------------------------------------
    stackframe[PROCESS_STACK_PTBASE] = (uint32)(pcb->pagetable);
     dbprintf('m',"debug stackframe[PROCESS_STATCK_PTBASE] 0x%x\n",stackframe[PROCESS_STACK_PTBASE]);
    stackframe[PROCESS_STACK_PTSIZE] = MEM_PTSIZE;
     dbprintf('m',"debug stackframe[PROCESS_STATCK_PTSIZE] 0x%x\n",stackframe[PROCESS_STACK_PTSIZE]);
    stackframe[PROCESS_STACK_PTBITS] = ((MEM_L1FIELD_FIRST_BITNUM)+(MEM_L1FIELD_FIRST_BITNUM <<16));
     dbprintf('m',"debug stackframe[PROCESS_STATCK_PTBITS] 0x%x\n",stackframe[PROCESS_STACK_PTBITS]);
  ...
  if (isUser) {
    ...
    //----------------------------------------------------------------------
     ...
     dbprintf('m',"debug initial user stack pointer is 0x%x\n",stackframe[PROCESS_STACK_USER_STACKPOINTER]);
   ...
   // dbprintf('m',"debug iar\n");
    stackframe[PROCESS_STACK_USER_STACKPOINTER] -= SIZE_ARG_BUFF;
    dbprintf('m',"debug stackframe[PROCESS_STACK_USER_STACKPOINTER]=0x%x\n",stackframe[PROCESS_STACK_USER_STACKPOINTER]);
    ...
    dbprintf('m',"debug stackframe[PROCESS_STACK_IAR]=0x%x\n",stackframe[PROCESS_STACK_IAR]);
...
intrs = DisableIntrs ();
printf("----------------------------Process_Fork()-----------------------\n");
//Get a free PCB for the new forked process
if(AQueueEmpty(&freepcbs)) {
   printf("FATAL error: no free processes!\n");
   exitsim();
}
...
ProcessSetStatus (pcb,PROCESS_STATUS_RUNNABLE);
RestoreIntrs (intrs);
//copy data from currentPCB to forking pcb
bcopy((char*)currentPCB,(char*)pcb,sizeof(PCB));

//printf("currentPCB->sysStack=0x%x,pcb->sysStack=0x%x\n",currentPCB->sysStackArea,pcb->sysStackArea);
//Marked both pagetable as readonly
for(i=0;i<MEM_PTSIZE;i++){
   ...
   if(pcb->pagetable[i]!=0){
   page=((pcb->pagetable[i])&(0xfffff000))>>(MEM_L1FIELD_FIRST_BITNUM);
  // printf("page number is 0x%x,pcb->pagetable[i]=0x%x\n",page,pcb->pagetable[i]);
   counter[page]+=1;
   printf("counter[0x%x] is %d\n",page,counter[page]);
   printf("Parent Page_number %d -> PTE:0x%x, Child Page_number %d -> PTE:0x%x\n",i,currentPCB->pagetable[i],i,pcb->pagetable[i]);
   }
}
//allocate a page for system stack
newpage=MemoryAllocPage ();
if(newpage == MEM_FAIL){
   printf("aFATAL:couldn't allocate memory - no free pages!\n");
   ProcessKill();
...
pcb->sysStackArea = newpage * (MEM_PAGESIZE);
//printf("new pcb->sysStackArea=0x%x\n",pcb->sysStackArea);
bcopy((char*)currentPCB->sysStackArea,(char*)pcb->sysStackArea,MEM_PAGESIZE);
//printf("pcb->currentSavedFrame[PROCESS_STACK_PTBASE]=0x%x,currentPCB->pagetable=0x%x\n",pcb->currentSavedFrame[PROCESS_STACK_PTBASE],currentPCB->pagetable);

//Fix something here
stackframe = (uint32*)((pcb->sysStackArea+(MEM_PAGESIZE))-4);
stackframe-= PROCESS_STACK_FRAME_SIZE;
//printf("pcb_stackframe=0x%x,currentPCB_stackframe=0x%x\n",stackframe,currentPCB->currentSavedFrame);
pcb->sysStackPtr = stackframe;
pcb->currentSavedFrame = stackframe;
stackframe[PROCESS_STACK_PTBASE] = (uint32)(pcb->pagetable);
//printf("pcb->currentSavedFrame[PROCESS_STACK_PTBASE]=0x%x,currentPCB->pagetable=0x%x\n",stackframe[PROCESS_STACK_PTBASE],currentPCB->pagetable);
