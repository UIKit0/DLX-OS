...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
   ...
   // printf("initialize test\n");
    ...
    pcbs[i].heapnode[0].address=0;
    pcbs[i].heapnode[0].order=7;
    pcbs[i].heapnode[0].size=4096;
    pcbs[i].heapnode[0].left=NULL;
    pcbs[i].heapnode[0].right=NULL;
    pcbs[i].heapnode[0].inuse=0;
    pcbs[i].heapnode[0].free_status=1;
    for(j=1;j<256;j++){
        pcbs[i].heapnode[j].inuse=1;
        pcbs[i].heapnode[j].free_status=1;
       }
...
void ProcessFreeResources (PCB *pcb) {
  ...
  for(i=0;i<(MEM_PTSIZE);i++){
     if(pcb->pagetable[i]!=0){
       //printf("free physical page 0x%x\n",pcb->pagetable[i]);
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  // printf("allocate test\n");
   ...
   pcb->npages +=1;
   pcb->pagetable[4]=MemorySetupPte(newPage);
 //  printf("allocate a page for heap, PTE is 0x%x\n",pcb->pagetable[i]);
 ...
 //  printf("allocate a page for system stack,sysStackArea is 0x%x\n",pcb->sysStackArea);
  ...
  dbprintf('m',"debug stackfram is 0x%x\n",stackframe);
