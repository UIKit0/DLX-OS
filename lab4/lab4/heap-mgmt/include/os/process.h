...
//node type for building up trees in function malloc and mfree
typedef struct node {
  int order;
  int address;
  int size;
  struct node *left;
  struct node *right;
  int free_status; //1 for free, 0 for taken
  int inuse;      //0 for inuse, 1 for free
}node;
