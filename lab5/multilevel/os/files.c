...
int MkDir(char *path, int permissions){
    ...
    if((len = getOneName(dst,path))== INVALID_PAT){
         printf("Error:Invalid Path\n");
         return FILE_FAIL;
    }
       type = DIR;
       ownerid = GetCurrentPid();
       permission = 077;

      if((handle = DfsInodeOpen(dst,handle,type,ownerid,permission))== DFS_FAIL){
       printf("Error:Cannot Open inode\n");
       return FILE_FAIL;
       }

       do{
          if((len = getOneName(dst,NULL))== INVALID_PATH){
          printf("Error:Invalid Path\n");
          return FILE_FAIL;
          }
       if((handle = DfsInodeOpen(dst,handle,type,ownerid,permission))==DFS_FAIL){
...
int FileOpen(char *filename,char *mode){
 ...
 // printf("InFileOpen, mode = %s\n",mode);
    ...
    if(!(dstrncmp(mode,mode_r,3)==0)&&(!(dstrncmp(mode,mode_w,3)==0))&&(!(dstrncmp(mode,mode_rw,3)==0))){
       printf("mode is %s\n",mode);
...
// printf("In FileRead\n");
    ...
    if(!(dstrncmp(files[handle].mode, mode_r,3)==0)&&(!(dstrncmp(files[handle].mode,mode_rw,3)==0))){
...
//printf("In FileWrite\n");
    ...
    if(!(dstrncmp(files[handle].mode,mode_w,3)==0)&&(!dstrncmp(files[handle].mode,mode_rw,3)==0)){
 //     printf("mode = %s , mode_w = %s\n",files[handle].mode,mode_w);
