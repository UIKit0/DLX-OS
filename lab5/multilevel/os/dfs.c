...
static dfs_dir dir[DFS_INODE_MAX_NUM];//all dirs
...
int DfsInodeFilenameExists(char *filename,int handle) {
   int i;
   int handler;
 //  printf("In DfsInodeFilename Exists\n");
   for (i=0; i<ENTRY_NUM;i++){
          if(dstrncmp(inodes[handle].dir[i].filename,filename,FILENAME_LENGTH) == 0){
            handler = inodes[handle].dir[i].inode; 
            return handler;
            }
       }
   return DFS_FAIL;
...
int InsertEntry(char *filename,int handle,int handler){
   int i,j;
   for(i=0;i<ENTRY_NUM;i++){
      if(inodes[handle].dir[i].filename[0]='\0'){
         for(j=0;j<FILENAME_LENTH;i++){
             inodes[handle].dir[i].filename[j]=filename[j];
         }
         inodes[handle].dir[i].inode = handler;
         return DFS_SUCCESS;
      }
   }
   printf("Error: Cannot insert entry for inode %d\n,handle");
   return DFS_FAIL;
}

void FreeEntry(int handle){
   int i;
   for(i=0;i<ENTRY_NUM;i++){
       inodes[handle].dir[i].filename[0]='\0';
       }
   inodes[handle].dir[i].inuse = 0;
}
//-----------------------------------------------------------------
