...
void main (int argc, char *argv[])
{
  ...
  mbox_t mbox_N2;           // Handle to the mailbox
  ...
  if (mbox_open(mbox_N2) == MBOX_FAIL) {
    Printf("spawn_me (%d): Could not open the mbox_N2!\n", getpid());
    Exit();
  }

  if (mbox_send(mbox_N2,sizeof(missile_code),(void *)&mc)==MBOX_FAIL) {
     Printf("Could not send message to mailbox_N2\n");
     Exit();
    }
 else Printf("Create 1 N2\n");
