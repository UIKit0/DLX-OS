...
// to happen).There should be 32 run Queues
static Queue	RunQueue[QUEUE_NUM];
...
void ProcessDecayEstcpuRun(PCB *pcb);
...
//-------------------------------------------------
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    // Set a number of initialization value
...
void ProcessFreeResources (PCB *pcb) {
 ...
 if( MboxCloseAllByPid(GetPidFromAddress(pcb))==MBOX_FAIL){
     printf("FATAL ERROR: could not close open mailbox connections!\n");
     exitsim();
   }
...
//----------------------------------------------------------------------
//      Helper Function
//      Those Helper Functions make ProcessSchedule function structural 
//      and easy to read
//----------------------------------------------------------------------
//
void ProcessRecalcPriority(PCB *pcb){
     pcb->priority =(int)( BASE_PRIORITY+pcb->estcpu/4+2*pcb->pnice);
    if (pcb->priority<BASE_PRIORITY){
     printf("FATAL ERROR: Priority for user process should not below BASE_PRIORITY!\n");
     pcb->priority=BASE_PRIORITY;
     exitsim();
    }// printf("Debug: ProcessRecalcPriority\n");
    }
...
int ProcessInsertRunning(PCB *pcb){
  ProcessSetStatus (pcb, PROCESS_STATUS_RUNNABLE);
    if(AQueueRemove(&(pcb->l)) != QUEUE_SUCCESS){
       printf("FATAL ERROR:could not remove process from Waiting Queue in ProcessInsertRunning!\n");
       return PROCESS_FAIL;
    }
    if((pcb->l = AQueueAllocLink(pcb))== NULL) {
       printf("FATAL ERROR: could not get Queue Link in ProcessInsertRunning!\n");
       return PROCESS_FAIL;
    }
    if(AQueueInsertLast(&RunQueue[WhichQueue(pcb)],pcb->l) != QUEUE_SUCCESS){
       printf("FATAL ERROR: could not insert pcb into runningQueue!\n");
       return PROCESS_FAIL;
    }//  printf("Debug:ProcessInsertRunning\n");
       return PROCESS_SUCCESS;
}
void ProcessEstcpuRun(PCB *pcb){ 
    if((pcb->switch_out_jiffies-pcb->switch_in_jiffies)>=11){
     pcb->estcpu=pcb->estcpu+1.0;
}//printf("Debug:ProcessEstcpuRun\n");
}

void ProcessDecayEstcpuSleep(PCB *pcb,int time_asleep_jiffies){
     if(pcb->sleep_jiffies>100){
      int i;
      int num_windows_asleep=(int)(pcb->sleep_jiffies/(TIME_PER_CPU_WINDOW*CPU_WINDOWS_BETWEEN_DECAYS));
      for(i=0;i<num_windows_asleep;i++){
      pcb->estcpu =pcb->estcpu *(2.0/3.0);
      }
     }// printf("Debug:ProcessDecayEstcpuSleep\n");  
    }
void ProcessDecayAllEstcpus(){
     int i=0;
     PCB *pcb=NULL;
     Link *l=NULL;
     if((ClkGetCurJiffies()%100>=0)&&(ClkGetCurJiffies()%100<=10)){  //Every 100 jiffies(0.1s)
//      printf("current jiffies %d\n",ClkGetCurJiffies());
      for(i=0;i<PRIORITY_RANGE/4;i++){
     ...
     }//printf("Debug:ProcessDecayAllEstcpusi\n");
    }
PCB *ProcessFindHighestPriorityPCB(){
     int i=0;
     Link *l=NULL;
     PCB *pcb=NULL;
     int Pid;
     while(AQueueEmpty(&RunQueue[i])&&(i<PRIORITY_RANGE/4)){
        i++;
     }
     if(i<PRIORITY_RANGE/4){
     l=AQueueFirst(&RunQueue[i]);
     pcb= AQueueObject(l);
   //  printf("in ProcessFindHighestPriorityPCB,highest priority %d\n",pcb->priority);
     return pcb;
     }
     else {
   //  printf("No running Process, Now creating an idle!!\n");
     Pid = ProcessFork(ProcessIdle,1000,0,0,NULL,0);
     idle= (PCB *)(pcbs+Pid);
   //  printf("idle priority %d\n",idle->priority);
     return idle;
     }
    }
int ProcessCountAutowake(){       //count autowake sleeping process
   int count=0;
   PCB *pcb=NULL;
   Link *l=NULL;
   if(!AQueueEmpty(&waitQueue)){
     ...
     while(l!=NULL){
     pcb=AQueueObject(l);
     if(pcb->sleep_jiffies>=0){  //Could wake up automatically
     if((pcb->sleep_start_jiffies+pcb->sleep_jiffies)<ClkGetCurJiffies()){
      ProcessDecayEstcpuSleep(pcb,pcb->sleep_jiffies);
      ProcessRecalcPriority(pcb);       //recalculate priority after waking up   
      ProcessWakeup(pcb);
     }
     else{
     count++;
   ...
   return count;
  printf("wake up %d processes\n",count);
}
int ProcessRunCheck(){     //Check if idle is the only running process
   PCB *pcb=NULL;
   Link *l=NULL;
   pcb=ProcessFindHighestPriorityPCB();
   l=AQueueFirst(&RunQueue[WhichQueue(pcb)]);
   if((pcb==idle)&&(AQueueNext(l)==NULL)){
    return 1;
   }
   return 0;
}
...
//	    (int)currentPCB, AQueueLength (&RunQueue));
  ...
  intrs = DisableIntrs ();              //Disable interrupt
 
  CheckAutowake=ProcessCountAutowake(); //Checking autowake sleeping processes while waking up some
  if (ProcessRunCheck()) {              //Only idle process exists
    if(AQueueEmpty(&waitQueue)){
     printf("No runnable processes and no waiting processes - exiting!\n");
     exitsim ();//NEVER RETURNS
     }
    if((!AQueueEmpty(&waitQueue))&&(CheckAutowake==0)) {
      printf("FATAL ERROR: no runnable processes, but there are sleeping processes waiting and cannot wake up!\n");
      l = AQueueFirst(&waitQueue);
      while (l != NULL) {
        pcb = AQueueObject(l);
  //      printf("Sleeping process %d: ", i++); printf("PID = %d\n", (int)(pcb - pcbs));
    ...
    if((!AQueueEmpty(&waitQueue))&&(CheckAutowake>0)) {
 //      printf ("Sleeping Process will auto wake!\n");
    }
  }
 
  ProcessDecayAllEstcpus();//Decaying estcpu time for all running process
 ...
 // AQueueMoveAfter(&runQueue, AQueueLast(&runQueue), AQueueFirst(&runQueue));
  ...
  currentPCB->switch_out_jiffies=ClkGetCurJiffies();  //record switch_out time of the old process
  currentPCB->cpu_jiffies+=currentPCB->switch_out_jiffies-currentPCB->switch_in_jiffies;//update cpu run time

 // printf("currentPCB cpu_jiffies= %d\n",currentPCB->cpu_jiffies);

  if(currentPCB->pinfo==1){printf(PROCESS_CPUSTATS_FORMAT,GetCurrentPid(),currentPCB->cpu_jiffies,0);}
  if(currentPCB->flags & PROCESS_STATUS_RUNNABLE){     //Check if CurrentPCB is running processi
   // printf("runnable process\n");
    if(currentPCB==idle){
   ...
   //  printf("idle priority %d\n",currentPCB->priority);
     pcb = ProcessFindHighestPriorityPCB();
       ...
       currentPCB = pcb;
       currentPCB->switch_in_jiffies=ClkGetCurJiffies(); //record swich_in time of a new process
       dbprintf ('p',"About to switch to PCB 0x%x,flags=0x%x @ 0x%x\n", (int)pcb, pcb->flags, (int)(pcb->sysStackPtr[PROCESS_STACK_IAR]));
        }
       else {
       ProcessEstcpuRun(currentPCB);
       ProcessRecalcPriority(currentPCB);
    //     printf("currentPCB priority,status %d,%x\n",currentPCB->priority,currentPCB->flags);
       ProcessInsertRunning(currentPCB);
       pcb = ProcessFindHighestPriorityPCB();
       currentPCB = pcb;
       currentPCB->switch_in_jiffies=ClkGetCurJiffies();
       dbprintf ('p',"About to switch to PCB 0x%x,flags=0x%x @ 0x%x\n", (int)pcb, pcb->flags, (int)(pcb->sysStackPtr[PROCESS_STACK_IAR]));
       }
     }   
    else if(currentPCB->flags & PROCESS_STATUS_YIELDING){
       ProcessInsertRunning(currentPCB);
       pcb = ProcessFindHighestPriorityPCB();
       currentPCB = pcb;
       currentPCB->switch_in_jiffies=ClkGetCurJiffies();
...
void ProcessSuspend (PCB *suspend) {
  ...
  suspend->sleep_start_jiffies=ClkGetCurJiffies(); //Set sleep start jiffies
...
void ProcessUserSleep(int seconds) {
  ...
  currentPCB->sleep_start_jiffies=ClkGetCurJiffies();
  currentPCB->sleep_jiffies=seconds*1000000/CLOCK_DEFAULT_RESOLUTION;//transfer seconds to jiffies
}
void ProcessIdle(){
    while(1){printf("idle");}
}
