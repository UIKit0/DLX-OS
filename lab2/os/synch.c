...
uint32 intrs;
  ...
  if(CondInit(&conds[con]) !=SYNC_SUCCESS) return INVALID_COND;
 //initial the aquired lock
  conds[con].lock = &locks[lock];
  return con;
}
...
int CondInit(Cond *con){
  ...
  if(AQueueInit (&con->waiting) != QUEUE_SUCCESS) {
    printf("FATAL ERROR: could not initialize con value waiting queue in CondInit!\n");
  exitsim();
  }
  return SYNC_SUCCESS;
}
...
int CondWait(Cond *con){
  ...
  //if curren process does not hold the lock,do not block on con value
  if((con->lock)->pid != GetCurrentPid()){
   dbprintf('s',"CondWait: Proc %d does not own cond %d\n",GetCurrentPid(),(int)(con-conds));
  RestoreIntrs(intrs);
  return 1;
 }

 //putting waiting process to sleep
  if ((l=AQueueAllocLink ((void *)currentPCB))==NULL){
    printf("FATAL ERROR: could not allocate link for cond queue in CondWait!\n");
  exitsim();
 }
  if (AQueueInsertLast(&con->waiting,l)!=QUEUE_SUCCESS){
    printf("FATAL ERROR: could not insert new link into lock waiting queue in CondWait!\n");
 }
 LockRelease(con->lock);
 RestoreIntrs(intrs);
 ProcessSleep();
 //Acquire lock after waking up
 LockAcquire(con->lock);
 return 0;
}
...
int CondSignal(Cond *con){
  ...
  intrs = DisableIntrs ();
  dbprintf ('I', "CondSignal: Old interrupt value was 0x%x.\n", intrs);
...
int CondBroadcast(Cond *c) {
    ...
    if (c->lock->pid != GetCurrentPid()){
       dbprintf('s', "CondBroadcast: Proc %d doesn't own cond %d\n",GetCurrentPid(),(int)(c-conds));
    return SYNC_FAIL;
    }

    while(!AQueueEmpty(&c->waiting)) {
      if (CondSignal(c) != 0){
      dbprintf('s',"CondBroadcast: Proc %d faile to signal con %d\n", GetCurrentPid(),(int)(c-conds));
      return SYNC_FAIL;
    }
  }
    return SYNC_SUCCESS;
